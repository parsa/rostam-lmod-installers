...
def query_latest_git_release(latest_url):
    ...
    return max(
        releases,
        key=lambda k: [int(x) for x in re.split('-|\.', k) if x.isdigit()])
...
def download_check_archive(archive_name, archive_url):
    ...
    assert http_result[
        0] == archive_name, f'Failed to download {archive_name} from "{archive_url}".'
...
def create_check_modulefile(git_version, module_base, dir_install):
    ...
    lmod_proc = subprocess.run(f'module show git/{git_version}',
                               shell=True,
                               capture_output=True,
                               text=True,
...
def check_module(module_name, git_version, git_executable):
    ...
    git_proc = subprocess.run(f'module load {module_name} && which git',
                              shell=True,
                              capture_output=True,
                              text=True,
                              env=dict(os.environ, LMOD_PAGER=''))
    ...
    ), f'Git executable loaded by Lmod is not the expected {git_version} verison. {git_proc.stdout}'
...
def main(module_base, module_dir):
    ...
    dir_install = os.path.join(os.path.expanduser('~/.local/git/'),
                               git_version)
    ...
    module_name = create_check_modulefile(git_version, module_base,
                                          dir_install)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description=
        'Download the latest Git tarball from kernel.org and generate a module.'
    )
