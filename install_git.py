...
def query_latest_git_release(latest_url):
    ...
    releases = re.findall('[a-z-\.0-9]+\.tar\.xz', latest_html)
    return max(releases, key=lambda k: [int(x) for x in re.split('-|\.', k) if x.isdigit()])


def install_check_git(dir_build, dir_install, git_version):
    # Configure Git
    subprocess.run(
        ['./configure', f'--prefix={dir_install}', '--with-editor=vim', '--quiet'], cwd=dir_build, check=True)

    # Install Git
    subprocess.run(
        ['make', 'install', f'-j{os.cpu_count()}', '-s'], cwd=dir_build, check=True)
    ...
    git_proc = subprocess.run(
        [git_executable, '--version'], capture_output=True, text=True)
    ...
    assert git_version in git_proc.stdout, f'Git executable {git_executable} is not the expected version: {git_proc.stdout}'
...
def create_check_modulefile(git_version, module_base, dir_install):
    ...
    lmod_proc = subprocess.run(f'module show git/{git_version}', shell=True,
...
def check_module(module_name, git_version, git_executable):
    ...
    git_proc = subprocess.run(f'module load {module_name} && which git', shell=True,
                              capture_output=True, text=True, env=dict(os.environ, LMOD_PAGER=''))
    assert git_proc.returncode == 0, f'Git executable is not in the PATH set by the Lmod modulefile.'
    assert os.path.samefile(git_proc.stdout.rstrip(
    ), git_executable), f'Git executable loaded by Lmod is not the expected {git_version} verison. {git_proc.stdout}'

    # Make sure the Git executable is in the path works and is the version we expected.
    git_proc = subprocess.run(f'module load {module_name} && git --version',
                              shell=True, capture_output=True, text=True, env=dict(os.environ, LMOD_PAGER=''))
    assert git_proc.returncode == 0, f'Failed to run the Git executable.\n' + \
        git_proc.stderr
...
def main(module_base, module_dir):
    ...
    git_version = '.'.join(
        [x for x in re.split('-|\.', archive_name) if x.isdigit()])
    print("\x1b[1K\rLatest Git version: " + git_version)

    print(f'Downloading {archive_name} from {latest_url}...',
          end='', flush=True)
    archive_url = urllib.parse.urljoin(latest_url, archive_name)
    http_result = urllib.request.urlretrieve(archive_url, archive_name)

    assert http_result[0] == archive_name, f'Failed to download {archive_name} from "{archive_url}".'
    assert os.path.isfile(
    ...
    dir_install = os.path.join(
        os.path.expanduser('~/.local/git/'), git_version)
    dir_build = '.'.join(archive_name.split('.')[:-2])

    print(
        f'Installing Git {git_version} in {dir_install}...', end='', flush=True)
    # Install Git
    ...
    module_name = create_check_modulefile(
        git_version, module_base, dir_install)
